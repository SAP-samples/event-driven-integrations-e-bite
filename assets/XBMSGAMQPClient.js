...
module.exports = class AMQPClient {
  ...
  /**
   * Connects the client to the event broker. Connection details should be specified in cds.requires.AMQP.credentials
   */
  connect = () => {
  ...
  registerSubscriber = (service, subscriptions = []) => {
    ...
    (Array.isArray(subscriptions) ? subscriptions : [subscriptions]).forEach((subscription) => {
      ...
      receiver.on("data", (msg) => {
        console.log(`${this.clientName} > Received on topic ${subscription}`);
